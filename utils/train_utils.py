...
读取配置文件
'''
def file2dict(filename, vis_cam=False):
    ...
    if vis_cam:
        return cfg_dict.get('model_cfg'),cfg_dict.get('train_pipeline'),cfg_dict.get("infer_pipeline"),cfg_dict.get('data_cfg'),cfg_dict.get('lr_config'),cfg_dict.get('optimizer_cfg')
    else:
...
def train(model, runner, lr_update_func, device, epoch, epoches, meta):
    ...
    with tqdm(total=len(runner.get('train_loader')),desc=f'Train: Epoch {epoch + 1}/{epoches}',postfix=dict,mininterval=0.3) as pbar:
        for iter, batch in enumerate(runner.get('train_loader')):            
            ...
            losses.get('loss').mean().backward()
            runner.get('optimizer').step()

            train_loss += losses.get('loss').mean().item()
