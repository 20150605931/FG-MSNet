...
def main(args):
    ...
    if args.kflod_validation > -1:
        test_annotations    = f'datas/testfold{args.kflod_validation}.txt'
    else:
    ...
    test_loader = DataLoader(
        test_dataset, shuffle=False, batch_size=data_cfg.get('batch_size'),
        num_workers=data_cfg.get('num_workers'), drop_last=False, pin_memory=True, collate_fn=collate
    )
    ...
    model.eval()
    with torch.no_grad():
        preds, targets, image_paths, time_cost = [],[],[], []
        with tqdm(total=len(test_datas)//data_cfg.get('batch_size')) as pbar:
            for _, batch in enumerate(test_loader):
                images, target, image_path = batch
                start = time.time()
                outputs = model(images.to(device),return_loss=False)   #output是最后的输出通过softmax层得到的概率
                end = time.time()
                ...
                pbar.update(1)
    print(f"time process: {np.mean(time_cost)} per/image")
