...
def parse_args():
    ...
    parser.add_argument('config', default='models/resnet/MS_ResWeightedPartNet50_FL_Tokken_ResPartNet.py', help='Config file')
    parser.add_argument(
        '--target-layers',
        default=[],  # 'backbone.resnet_conv.7.2.bn3'
        nargs='+',
        type=str,
    ...
    parser.add_argument('--device', default='cuda:0', help='Device to use cpu')
    parser.add_argument(
        '--vit-like',
        action='store_true',
...
def visualize_single_img(args, model, val_pipeline, img_path, save_path):
    # apply transform and perpare data
    data, src_img = apply_transforms(img_path, val_pipeline)
...
def main():
    args = parse_args()
    model_cfg,train_pipeline,val_pipeline,data_cfg,lr_config,optimizer_cfg = file2dict(args.config, vis_cam=True)
    ...
    if os.path.isfile(args.img):
        ...
        visualize_single_img(args, model, val_pipeline, args.img, save_path)
    else:
        filelist = os.listdir(args.img)
        for iimg in filelist:
            img_path = os.path.join(args.img, iimg)
            save_path=os.path.join(args.save_path, iimg)
            visualize_single_img(args, model, val_pipeline, img_path, save_path)
