...
class WeightedPartClsHead_MSP(ClsHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(WeightedPartClsHead_MSP, self).__init__(init_cfg=init_cfg, *args, **kwargs)
        ...
        for i in range(2):
            ...
            setattr(self, name, BottleClassifier(
                10240, self.num_classes, relu=True,
    ...
    #     global_logits, part_logits_c, part_logits_r, avg_logits = score_list
    #     num_samples = len(global_logits)

    #     losses[f'loss_global'] = self.compute_loss(
    #         avg_logits, gt_label, avg_factor=num_samples, **kwargs)
    #     loss_all = losses[f'loss_global']

    #     # losses[f'loss_global_mamba'] = self.compute_loss(
    #     #     global_score, gt_label, avg_factor=num_samples, **kwargs)
    #     # loss_all= losses[f'loss_global'] + losses[f'loss_global_mamba']
        
    #     if self.cal_acc:
    #         # compute accuracy
    #         with torch.no_grad():
    #             global_acc = self.compute_accuracy(global_logits, gt_label)
    ...
    def loss(self, score_list, gt_label, **kwargs):
            ...
            global_score, part_scores = score_list
            part_score, avg_logits = part_scores
...
############# original #################
    ...
    #     if features_part.dim() ==2:
    #         features_part=features_part.unsqueeze(0)
    ...
    def pre_logits_o(self, features):
        features_part_c, features_part_r, features_global = features
        
        feature_list = [features_global]

        features_c = torch.FloatTensor()
        features_r = torch.FloatTensor()

        if features_part_c.dim() ==2:
            features_part_c=features_part_c.unsqueeze(0, dim=0)
        for i in range(self.part_num):
            if self.part_num == 1:
                ...
                features_c = torch.cat([features_c,features_ci], dim = 1)
        ...
        for j in range(self.part_num):
            if self.part_num == 1:
                ...
                features_ri = features_part_r[:, :, :, i]
                features_r = torch.cat([features_r,features_ri], dim = 1)
    ...
    #     avg_logits = 0
    #     logits_list = []
    #     #global_logits = self.fc(feature_list[0])  #把全局池化得到的向量送入fc层   # o_h
    #     #global_logits = self.MambaBlock(feature_list[0])
       
        
    #     global_features = self.mamba_model_g(torch.squeeze(feature_list[0],3))
    #     global_logits = self.fc(global_features.squeeze(2))
    #     # classifier_g = getattr(self, 'classifier_g')
    #     # global_logits = classifier_g(global_features.squeeze(2))

    #     part_features_c = self.mamba_model_p(feature_list[1])
    #     #part_logits_c = self.fc_p(part_features_c)
    #     classifier_0 = getattr(self, 'classifier0')
    #     part_logits_c = classifier_0(part_features_c.squeeze(2))
    #     logits_list.append(part_logits_c)

    #     part_features_r = self.mamba_model_p(feature_list[2])
    #     #part_logits_r = self.fc_p(part_features_r)
    #     classifier_1 = getattr(self, 'classifier1')
    #     part_logits_r = classifier_1(part_features_r.squeeze(2))
    #     logits_list.append(part_logits_r)
    ...
    #     #     avg_logits += self.w[i] * logits_i
    #     avg_logits = self.w[0] * part_logits_c + self.w[1] * part_logits_r
    ...
    def get_logits(self, feature_list):
        ...
        #global_features = self.mamba_model_g(torch.squeeze(feature_list[0],3))
        global_logits = self.fc(feature_list[0].squeeze(3).squeeze(2))
        # classifier_g = getattr(self, 'classifier_g')
        # global_logits = classifier_g(global_features.squeeze(2))

        #part_features_c = self.mamba_model_p(feature_list[1])
        #part_logits_c = self.fc_p_c(feature_list[1].squeeze(2))
        classifier_0 = getattr(self, 'classifier0')
        part_logits_c = classifier_0(feature_list[1].squeeze(2))
        logits_list.append(part_logits_c)

        #part_features_r = self.mamba_model_p(feature_list[2])
        #part_logits_r = self.fc_p_r(feature_list[2].squeeze(2))
        classifier_1 = getattr(self, 'classifier1')
    ...
    def forward_train(self, x, gt_label):
        ...
        losses = self.loss([global_logits, (logits_list, avg_logits)], gt_label)
